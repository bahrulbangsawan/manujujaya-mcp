...
const EXCLUSIONS: CoverageEntry[] = [
  ...
  },
  {
    ...
    auth: "www-csrf",
    safety: "write",
    implModule: "src/session/qasir-session.ts",
    testFile: "tests/unit/session.test.ts",
    status: "session-only",
    exclusionReason:
      "Host-only session bootstrap; PIN never accepted from model. Pre-provisioned secrets used instead.",
  },
  {
    sourceDocument: "auth-login.md",
    method: "POST",
    ...
    testFile: null,
    status: "session-only",
    exclusionReason: "Auth bootstrap helper; not exposed via Code Mode",
  },
  {
    sourceDocument: "auth-login.md",
    method: "POST",
    ...
    testFile: null,
    status: "session-only",
    exclusionReason: "Auth bootstrap; not used with pre-provisioned session",
  },
  {

...
### Community 24 - "widgets-define.test.ts"
Cohesion: 0.09
...
### Community 71 - "OrderDetailSheet.tsx"
Cohesion: 0.15
...
- **Why does `AppError` connect `AppError` to `connect/routes.ts`, `codemode-harness.ts`, `contract.ts`, `transactions.ts`, `run.ts`, `stock.ts`, `widgets-sales.test.ts`, `define.ts`, `session/types.ts`, `login-continue.ts`, `server.ts`, `validate.ts`, `debts.ts`, `ExecutionBudget`, `widgets-define.test.ts`, `MutationApprovalsDO`, `connect.test.ts`, `text.ts`, `QasirDispatcher`, `session-store.ts`, `scenarios.test.ts`, `qasir-dates.ts`, `dispatcher.test.ts`, `purchases.ts`, `mutation-tool.ts`, `codes.ts`, `paste.ts`?**
  _High betweenness centrality (0.109) - this node is a cross-community bridge._
- **Why does `ErrorCodes` connect `codes.ts` to `connect/routes.ts`, `AppError`, `codemode-harness.ts`, `contract.ts`, `transactions.ts`, `run.ts`, `stock.ts`, `widgets-sales.test.ts`, `define.ts`, `session/types.ts`, `server.ts`, `validate.ts`, `debts.ts`, `widgets-define.test.ts`, `connect.test.ts`, `text.ts`, `session-store.ts`, `scenarios.test.ts`, `qasir-dates.ts`, `dispatcher.test.ts`, `purchases.ts`, `mutation-tool.ts`, `paste.ts`, `dispatcher-transport.test.ts`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `log()` connect `connect/routes.ts` to `AppError`, `mutation-tool.ts`, `run.ts`, `QasirDispatcher`, `session-store.ts`, `define.ts`, `server.ts`, `SessionStore`?**
...
- **Should `connect/routes.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.052444444444444446 - nodes in this community are weakly interconnected._
- **Should `query.test.tsx` be split into smaller, more focused modules?**
